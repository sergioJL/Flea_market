...
jQuery(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = jq('#actor7 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	jq('#actor7').width(w * n);
	jq('#actor7 li').each(function(i) {
		str += '<span></span>'
	});
	jq('#numInner').width(numWidth).html(str);
	jq('#imgPlay7 .mc').width(numWidth);
	jq('#imgPlay7 .num').css('left', _left);
	...
	jq('.book_ban #imgPlay7 .next_t').click(function() {
		slide(1)
	});
	jq('.book_ban #imgPlay7 .prev_t').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if (jq('#actor7').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				jq('#actor7').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if (jq('#actor7').css('marginLeft') != -i * w + 'px') {
			jq('#actor7').css('marginLeft', -i * w + 'px');
			jq('#actor7').fadeOut(0,
			function() {
				jq('#actor7').fadeIn(500)
			})
		}
	}
	function start() {
