...
jQuery(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = jq('#actor8 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	jq('#actor8').width(w * n);
	jq('#actor8 li').each(function(i) {
		str += '<span></span>'
	});
	jq('#numInner').width(numWidth).html(str);
	jq('#imgPlay8 .mc').width(numWidth);
	jq('#imgPlay8 .num').css('left', _left);
	...
	jq('.mus_ban #imgPlay8 .next_t').click(function() {
		slide(1)
	});
	jq('.mus_ban #imgPlay8 .prev_t').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if (jq('#actor8').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				jq('#actor8').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if (jq('#actor8').css('marginLeft') != -i * w + 'px') {
			jq('#actor8').css('marginLeft', -i * w + 'px');
			jq('#actor8').fadeOut(0,
			function() {
				jq('#actor8').fadeIn(500)
			})
		}
	}
	function start() {
