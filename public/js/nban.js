...
jQuery(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = jq('#actor9 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	jq('#actor9').width(w * n);
	jq('#actor9 li').each(function(i) {
		str += '<span></span>'
	});
	jq('#numInner').width(numWidth).html(str);
	jq('#imgPlay9 .mc').width(numWidth);
	jq('#imgPlay9 .num').css('left', _left);
	...
	jq('.work_ban #imgPlay9 .next_t').click(function() {
		slide(1)
	});
	jq('.work_ban #imgPlay9 .prev_t').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if (jq('#actor9').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				jq('#actor9').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if (jq('#actor9').css('marginLeft') != -i * w + 'px') {
			jq('#actor9').css('marginLeft', -i * w + 'px');
			jq('#actor9').fadeOut(0,
			function() {
				jq('#actor9').fadeIn(500)
			})
		}
	}
	function start() {
